...
\begin{itemize}
  \item $\lambda_{\text{peak}} = 0{,}30$ clientes/min durante las horas pico,
  \item $\lambda_{\text{off-peak}} = 0{,}17$ clientes/min en el resto de la jornada.
\end{itemize}

\begin{quote}
\textbf{Nota sobre los parámetros asumidos.}
El enunciado no especifica valores concretos para las tasas de llegada ni
para la proporción de clientes por tipo de producto.
Los valores $\lambda_{\text{peak}} = 0{,}30$ y $\lambda_{\text{off-peak}} = 0{,}17$
clientes/min se calibran a partir de referencias típicas de puestos de comida
rápida en centros comerciales~\citep{banks2010discrete}: aproximadamente
18 clientes/hora en pico y 10 fuera de pico, con una razón
$\lambda_{\text{peak}} / \lambda_{\text{off-peak}} \approx 1{,}76$.
Análogamente, se asume $p = 0{,}5$ (igual proporción de clientes de sándwich
y sushi) al no disponerse de datos históricos del local.
El impacto de variaciones en $\lambda_{\text{peak}}$ y $\lambda_{\text{off-peak}}$
se analiza en la \cref{sec:sensibilidad}.
\end{quote}
...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    Entrada & Tasa de llegadas (pico)     & $\lambda_{\text{peak}} = 0{,}30$      & clientes/min \\
    Entrada & Tasa de llegadas (fuera)    & $\lambda_{\text{off-peak}} = 0{,}17$  & clientes/min \\
