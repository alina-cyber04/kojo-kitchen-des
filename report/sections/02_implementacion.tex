...
\begin{equation}
  ...
  \label{eq:lcg}
\end{equation}
con los parámetros popularizados por Press et al.\ en \textit{Numerical Recipes}~\citep{press2007numerical}:
\[
  A = 1\,664\,525, \quad C = 1\,013\,904\,223, \quad M = 2^{32}.
\]
Estos parámetros satisfacen los criterios de Hull--Dobell~\citep{knuth1997art} para período
...
% ---------------------------------------------------------------------------
\subsection[Streams independientes y Common Random Numbers (CRN)]{Streams independientes y\\ Common Random Numbers~(CRN)}
% ---------------------------------------------------------------------------
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{9cm}}
    ...
    \midrule
    \texttt{ARRIVAL}        & Arribo de un nuevo cliente. Se determina su tipo
    ...
    \addlinespace
    \texttt{DEPARTURE}        & Fin del servicio de un cliente. El servidor queda
                              libre. Si la cola no está vacía se atiende al
                              siguiente cliente. Si el empleado está marcado
                              para retiro, se elimina. \\
    \addlinespace
    \texttt{PEAK\_START}   & Comienzo de una hora pico. En el Escenario B se
                              añade el empleado extra; si hay cola, comienza a
                              atender de inmediato. \\
    \addlinespace
    \texttt{PEAK\_END}      & Fin de una hora pico. En el Escenario B el empleado
                              extra es marcado para retiro si está ocupado, o
                              retirado inmediatamente si está libre. \\
    \addlinespace
    \texttt{END\_OF\_DAY}& Cierre del día (t = 660 min). Los servicios en curso
...
\State Crear empleados base; programar primera \texttt{ARRIVAL} y eventos de pico
\While{la lista de eventos futuros (FEL) no está vacía}
  ...
  \If{$e$ es \texttt{ARRIVAL}}
    ...
    \If{existe servidor libre}
      \State Asignar cliente al servidor; programar \texttt{DEPARTURE} en $t + s$
    \Else
      \State Encolar cliente; registrar cambio de cola
    \EndIf
    \State Programar siguiente \texttt{ARRIVAL} en $t + \operatorname{Exp}(\lambda(t))$
  \ElsIf{$e$ es \texttt{DEPARTURE}}
    \State Liberar servidor; registrar $\mathtt{departure\_time} = t$
    \If{cola no vacía \textbf{y} $t \le T$}
      \State Desencolar siguiente cliente; asignarlo; programar \texttt{DEPARTURE}
    \EndIf
  \ElsIf{$e$ es \texttt{PEAK\_START} \textbf{y} $\mathtt{extra\_staff} > 0$}
    \State Crear empleado extra; si hay cola, asignarle cliente inmediatamente
  \ElsIf{$e$ es \texttt{PEAK\_END} \textbf{y} $\mathtt{extra\_staff} > 0$}
...
\[
  ...
  \end{cases}
\]
En la implementación, cada evento \texttt{ARRIVAL} programa el siguiente
